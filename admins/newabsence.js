...
import {
  ...
  doc, getDoc, Timestamp, updateDoc, deleteDoc, serverTimestamp
...
export async function mountTodayAbsenceSheet(host, hooks) {
  ...
  if (!res.ok) throw new Error(`newabsence.css: HTTP ${res.status}`);
  ...
  sheetDoc.append(fa, style, tpl.content);
      ...
      function buildMorningLatesMap(snapshot) {
        ...
        snapshot.forEach(docSnap => {
          ...
          map[classKey].details.push({
            id: docSnap.id,
            studentId: (data.studentUid || data.studentId || '').toString().trim(),
      ...
      // One row per student who has an absence/late that day.
      function buildStudentRows(){
        const byKey = new Map();
        const touch = (st, cn) => {
          const key = st.id ? `id:${st.id}` : `name:${cn}:${st.name}`;
          if (!byKey.has(key)) {
            byKey.set(key, { id: st.id || '', name: st.name || '—', className: cn, num: st.num, events: new Map() });
          }
        ...
        for (const cn of ALL_CLASSES) {
          for (let lesson = 1; lesson <= 7; lesson++) {
            ...
            if (!cell) continue;
            for (const st of cell.students) {
              touch(st, cn).events.set(lesson, { lesson, status: 'absent', cell });
            }
            for (const st of cell.lateStudents) {
              const row = touch(st, cn);
              if (!row.events.has(lesson)) row.events.set(lesson, { lesson, status: 'late', cell });
            }
          }
        }
        ...
        for (const cn of ALL_CLASSES) {
          for (const d of morningLatesMap[cn]?.details || []) {
            const st = { id: d.studentId, name: d.studentName };
            touch(st, cn).morning = d;
          }
        }

        const byClass = new Map(ALL_CLASSES.map((cn) => [cn, []]));
        for (const row of byKey.values()) byClass.get(row.className)?.push(row);
        for (const list of byClass.values()) {
          list.sort((a, b) => {
            const an = Number.isFinite(a.num), bn = Number.isFinite(b.num);
      ...
      function renderGrid(){
        ...
        const byClass = buildStudentRows();
        let anyRow = false;

        for (const [cn, rows] of byClass) {
          rows.forEach((row, idx) => {
            anyRow = true;
            const tr = document.createElement('tr');
            if (idx === 0) tr.className = 'class-first';

            if (idx === 0) {
              const tdClass = document.createElement('td');
              tdClass.className = 'class-col class-group';
              tdClass.rowSpan = rows.length;
              tdClass.innerHTML = `<div class="class-label">${escapeHtml(toArabicDigits(cn))}</div>`;
            ...
            nameBtn.type = 'button';
            nameBtn.className = 'student-link';
            ...
            tdLessons.className = 'lessons-col';
            const dots = document.createElement('div');
            dots.className = 'lesson-dots';
            const events = [...row.events.values()].sort((a, b) => a.lesson - b.lesson);
            if (row.morning) {
              const chip = document.createElement('button');
              chip.type = 'button';
              chip.className = 'morning-chip';
              chip.innerHTML = '<i class="fas fa-sun" aria-hidden="true"></i>تأخير الصبح';
              chip.addEventListener('click', (e) => {
                e.stopPropagation();
                openMorningLateModal(row, cn);
              });
              dots.appendChild(chip);
            }
            if (!events.length && !row.morning) {
              dots.innerHTML = '<span class="none">—</span>';
            }
            for (const ev of events) {
              const dot = document.createElement('button');
              dot.type = 'button';
              dot.className = `lesson-dot ${ev.status}`;
              dot.textContent = String(ev.lesson);
              dot.title = `${ev.status === 'absent' ? 'غياب' : 'تأخير'} - الحصة ${toArabicDigits(ev.lesson)}`;
              dot.addEventListener('click', (e) => {
                e.stopPropagation();
                openStudentLessonModal(row, cn, ev);
              });
              dots.appendChild(dot);
            }
            tdLessons.appendChild(dots);
        ...
        if (!anyRow) {
          const tr = document.createElement('tr');
          tr.innerHTML = `<td colspan="3" class="muted" style="padding:28px 12px;font-weight:800;text-align:center">${
            'لا يوجد غياب أو تأخير في هذا اليوم.'
          }</td>`;
          gridBody.appendChild(tr);
        }
      ...
      // currently on screen (below the sticky header).
      function centerClassLabels(){
        const viewport = tableScroll.getBoundingClientRect();
        const thead = tableScroll.querySelector('thead');
        const top = viewport.top + (thead ? thead.getBoundingClientRect().height : 0);
        const bottom = viewport.bottom;
        gridBody.querySelectorAll('td.class-group').forEach((td) => {
          const label = td.firstElementChild;
          if (!label) return;
          const r = td.getBoundingClientRect();
          const visTop = Math.max(r.top, top);
          const visBottom = Math.min(r.bottom, bottom);
          if (visBottom <= visTop) return;
          label.style.top = ((visTop + visBottom) / 2 - r.top) + 'px';
        });
      }
      let centerRaf = 0;
      const scheduleCenter = () => {
        if (centerRaf) return;
        centerRaf = requestAnimationFrame(() => { centerRaf = 0; centerClassLabels(); });
      };
      tableScroll.addEventListener('scroll', scheduleCenter, { passive: true });
      window.addEventListener('resize', scheduleCenter);


      async function openStudentLessonModal(row, className, ev){
        const niceDate = toArabicDigits(fmtDateLong(selectedDate));
        const t = LESSON_TIMES[ev.lesson - 1];
        const isAbsent = ev.status === 'absent';
        const cell = ev.cell;

        modalTitle.textContent = row.name;
        modalSubtitle.textContent = `${toArabicDigits(className)} | ${niceDate}`;
        modalFooter.innerHTML = '';
        modalBody.innerHTML = `
          <div class="late-info-grid">
            <div class="info-item"><div class="label">الحالة</div><div class="value" style="color:var(${isAbsent ? '--red' : '--yellow'})">${isAbsent ? 'غائب' : 'متأخر'}</div></div>
            <div class="info-item"><div class="label">الحصة</div><div class="value">${toArabicDigits(ev.lesson)}</div></div>
            <div class="info-item"><div class="label">الوقت</div><div class="value">${t ? `${t.start} - ${t.end}` : '—'}</div></div>
            <div class="info-item"><div class="label">سجّلها</div><div class="value" id="lessonTakenBy">جاري التحميل…</div></div>
          </div>
        `;
        openModal();
        ...
        if (cell.sessionId) {
          ...
          note.style.cssText = 'font-weight:800;text-align:center;width:100%';
          note.textContent = 'سجل قديم — لا يمكن تعديله من هنا.';
          modalFooter.appendChild(note);
        }

        const uids = [...cell.teacherUids].filter(Boolean);
        const names = uids.length ? await Promise.all(uids.map((u) => getTeacherName(u))) : [];
        const el = sheetDoc.getElementById('lessonTakenBy');
        if (el && lessonModal.classList.contains('open')) {
          const uniq = [...new Set(names.filter(Boolean))];
          el.textContent = uniq.length ? uniq.join('، ') : 'غير محدد';
        }
      }
      ...
      // Morning late (تأخير الصبح) for one student: view, edit, or delete.
      function openMorningLateModal(row, className) {
        const d = row.morning;
        if (!d) return;
        const niceDate = toArabicDigits(fmtDateLong(selectedDate));

        modalTitle.textContent = row.name;
        modalSubtitle.textContent = `تأخير الصبح | ${toArabicDigits(className)} | ${niceDate}`;

        const showView = () => {
          const mins = Number(d.minutesLate);
          modalBody.innerHTML = `
            <div class="late-info-grid">
              <div class="info-item"><div class="label">الحالة</div><div class="value" style="color:var(--yellow)">متأخر صباحاً</div></div>
              <div class="info-item"><div class="label">وقت الوصول</div><div class="value">${escapeHtml(toArabicDigits(d.time))}</div></div>
              ${mins > 0 ? `<div class="info-item"><div class="label">مدة التأخير</div><div class="value">${toArabicDigits(mins)} دقيقة</div></div>` : ''}
              <div class="info-item"><div class="label">سجّله</div><div class="value">${escapeHtml(d.createdByName || 'غير محدد')}</div></div>
              <div class="info-item" style="grid-column:1/-1"><div class="label">السبب</div><div class="value">${escapeHtml(d.reason || '—')}</div></div>
            </div>`;
          modalFooter.innerHTML = '';
          ...
          delBtn.addEventListener('click', removeLate);
          modalFooter.appendChild(delBtn);
        };

        const showEdit = () => {
          const timeVal = /^\d{1,2}:\d{2}$/.test(d.time || '') ? d.time.padStart(5, '0') : '';
          modalBody.innerHTML = `
            <div class="edit-form">
              <label>وقت الوصول<input type="time" id="mlTime" value="${escapeHtml(timeVal)}"></label>
              <label>السبب<input type="text" id="mlReason" value="${escapeHtml(d.reason || '')}" placeholder="اختياري"></label>
            </div>`;
          modalFooter.innerHTML = '';
          ...
          saveBtn.addEventListener('click', async () => {
            const time = modalBody.querySelector('#mlTime').value;
            const reason = modalBody.querySelector('#mlReason').value.trim();
            if (!time) { modalBody.querySelector('#mlTime').focus(); return; }
            saveBtn.disabled = true;
            try {
              await updateDoc(doc(db, 'morningLates', d.id), { time, reason, updatedAt: serverTimestamp() });
              d.time = time;
              d.reason = reason;
              renderGrid();
              showView();
              ...
              alert('تعذّر الحفظ. تحقق من الصلاحيات أو الشبكة.');
            }
          });
        ...
        const removeLate = async () => {
          if (!confirm(`حذف تأخير الصبح للطالب ${row.name}؟ لا يمكن التراجع.`)) return;
          try {
            await deleteDoc(doc(db, 'morningLates', d.id));
            const entry = morningLatesMap[className];
            if (entry) {
              entry.details = entry.details.filter((x) => x.id !== d.id);
              entry.count = entry.details.length;
            }
            closeModal();
            renderGrid();
          } catch (e) {
            console.error(e);
            alert('تعذّر الحذف. تحقق من الصلاحيات أو الشبكة.');
          }
        };
      ...
      // Skipped when the grid is under a minute old — opening the sheet
      // just after its background preload would otherwise re-read every
      ...
      // (it stays mounted between opens, and is preloaded before the first).
